...
SURFACE = {
    ...
    "I": ("character_profile", "Read or maintain the fictional character's accumulated profile notes. read=true or empty arguments reads accepted notes and pending candidates. content creates a candidate; aspect identifies its dimension; limit bounds the result count. promote proposes a supported candidate after the store's required evidence from three distinct review dates. supersedes identifies a prior note in the same dimension. Keep the existing evidence, replacement and revision rules. These are authored story facts and character observations."),
